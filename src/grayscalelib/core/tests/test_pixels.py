...
def test_init(pixels_subclass):
    ...
    for fbits, black in itertools.product([0, 1, 2], range(-3, 3)):
        for white, limit in itertools.product(range(black, black+3), range(black, black+9)):
            delta = (white - black)
            ...
            for i, j in itertools.product(*tuple(range(s) for s in px.shape)):
...
def test_getitem(pixels_subclass):
    ...
    axes = (0, 1, 2, 3)
    for shape in itertools.chain(*[itertools.permutations(axes, k) for k in range(5)]):
        rank = len(shape)
        ...
        for index in itertools.product(* [slices] * rank):
            selection = px[index]
